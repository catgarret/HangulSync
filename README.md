...
2. **한쪽 Mac에서만** 설정의 **새 Mac 페어링**을 누릅니다.
   양쪽에서 동시에 누를 필요가 없으며, 앱이 Bonjour와 Tailscale 경로를 자동으로 찾습니다.
3. 자동으로 찾지 못하면 어느 한쪽에서 **초대 코드 만들기**를 누릅니다.
   자동 복사된 코드를 2분 안에 다른 Mac의 **초대 코드 입력**에 붙여 넣습니다.
4. 양쪽에 표시된 6자리 코드가 같은지 확인한 뒤 양쪽에서 **승인**합니다.
   **이 기기를 계속 신뢰**를 켜면 이후에는 다시 확인하지 않습니다.
5. macOS가 로그인 Keychain 접근을 요청하면 앱 이름이 HangulSync인지 확인합니다.
