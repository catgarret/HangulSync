...
2. **只在一台 Mac 上**按下設定中的**配對新的 Mac**。
   不必兩台同時按下。
3. 選擇**自動尋找**以搜尋 Bonjour 與 Tailscale。
4. 尋找失敗時，在任一台選擇**建立邀請碼**。
   兩分鐘內將已複製的邀請碼貼到另一台的**輸入邀請碼**。
5. 確認兩台 Mac 顯示相同的六位數代碼，然後在兩台裝置上核准。
   保持**永遠信任此裝置**開啟，日後便不需再次確認。
6. 如果 macOS 要求存取登入 Keychain，請確認應用程式名稱為 HangulSync。
