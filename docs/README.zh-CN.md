...
- **连接批准**
  新连接在用户批准前不会交换输入状态
- **加密互联网中继**
  已配对的 Mac 在无法直接连接时通过 ntfy 交换端到端加密的状态
...
1. 在两台 Mac 上运行 HangulSync。
2. **只在一台 Mac 上**点击设置中的**配对新 Mac**。
   无需两台同时点击，应用会自动查找 Bonjour 和 Tailscale 路径。
3. 自动查找失败时，在任意一台上选择**创建邀请码**。
   两分钟内将已复制的邀请码粘贴到另一台的**输入邀请码**。
4. 确认两台 Mac 显示相同的六位代码，然后在两台设备上批准。
   保持**始终信任此设备**开启，今后无需再次确认。
5. 如果 macOS 请求访问登录 Keychain，请确认应用名称为 HangulSync。
   输入当前 Mac 的登录密码，然后选择**始终允许**或**允许**。

密码只应输入到 macOS Keychain 对话框，不要输入到 HangulSync 窗口。
取消提示后，安全配对和互联网中继将无法工作。
