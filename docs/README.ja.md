...
1. 両方の Mac で HangulSync を起動します。
2. 2台のうち**片方の Mac だけで自動で検索**を押します。
   もう片方ではボタンやウインドウを操作せず、HangulSync を起動したままにします。
3. 検索する Mac には進行状況が表示され、
   もう片方には接続承認画面が自動で表示されます。
4. 見つからない場合は、どちらかで**招待コードを作成**を選びます。
   コピーされたコードを2分以内にもう一方の**招待コードを入力**へ貼り付けます。
5. 両方に同じ6桁コードが表示されていることを確認し、両方で承認します。
   **このデバイスを常に信頼する**を有効にすると、次回から確認は不要です。
   次回以降は両方でアプリを起動するだけで自動的に再接続します。
6. macOS がログイン Keychain へのアクセスを求めた場合、
   アプリ名が HangulSync であることを確認します。
   Mac のログインパスワードを入力し、**常に許可**または**許可**を選びます。

パスワードは HangulSync ではなく macOS の Keychain ダイアログにのみ入力します。
キャンセルすると安全なペアリングとインターネットリレーは動作しません。
