...
2. **片方の Mac だけで**設定の**新しい Mac をペアリング**を押します。
   両方で押す必要はなく、Bonjour と Tailscale は自動で検索されます。
3. 自動検索で見つからない場合は、どちらかで**招待コードを作成**を選びます。
   コピーされたコードを2分以内にもう一方の**招待コードを入力**へ貼り付けます。
4. 両方に同じ6桁コードが表示されていることを確認し、両方で承認します。
   **このデバイスを常に信頼する**を有効にすると、次回から確認は不要です。
5. macOS がログイン Keychain へのアクセスを求めた場合、
